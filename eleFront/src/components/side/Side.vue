<template>
    <div class="side">
        <!-- 地址选择 -->
        <div class="side_main">
            <vHead class="main_header" title="选择收货地址" @back="$emit('sback')">
        ...
        <div class="side_city" v-show="show.city">
            ...
            <!-- 所有城市列表的显示 -->
            <div class="city_main" v-show="!searchStr">
                <!-- 当前城市列表的显示 -->
                <div class="current_box">
                    <span>当前城市</span>
                    <li class="add_list">{{ currentCity }}</li>
                </div>
                <van-index-bar>
                    <ul v-for="(items, k, i) in cityName" :key="i">
...
.side {
    ...
    .side_main {
        .main_header {
            .add {
                position: absolute;
                right: 0.266666rem; //10px
                top: 0;
                line-height: 1.466666rem; //55px
                color: #fff;
        ...
        .main_search {
            ...
            .search_input {
                ...
                .input {
                    padding: 0.133333rem; //5px
                    width: 6.666666rem; //250px
                    color: #999;
                }
            }
    ...
    .side_city {
        ...
        width: 100%;
        .city_header {
            padding-bottom: 0.026666rem; //10px
        }
        .city_search {
            margin: 0.266666rem 0.933333rem; //10px 35px
            background-color: #fff;
            padding: 0.266666rem 0.533333rem;
            border-radius: 2.666666rem; //10px
            color: #999;
            font-size: 0.32rem;
            font-weight: 600;
        }
